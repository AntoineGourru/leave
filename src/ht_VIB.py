...
HIDDEN_CHANNELS = [[256, 128], [256, 256]]
...
BETAS = [1e-11, 1e-10, 1e-9, 1e-8, 1e-7]
L = [1, 5, 10]
...
for i in seeds:
    ...
    for encoder, alpha, lr, epoch, chan, beta, num_lay, l in \
            itertools.product(ENCODER, ALPHAS, LR, EPOCHS, HIDDEN_CHANNELS, BETAS, NUM_LAYERS, L):
        print("=" * 60)
        ...
        for e in range(epoch):
            ...
            loss, _, _ = model.loss_vib(dataset_train.x, dataset_train.edge_index, x_1_train, x_2_train, y_train,
                                        s_train, criterion, alpha, beta=beta, L=l)
            loss.backward()
            optimizer.step()
            if e % 10 == 0:
        ...
        with open('results/'+dataset_name+'/'+OUTPUT_FILE, "a") as f:  # 'a' = append mode to not overwrite
            params = f"ALPHA={alpha:.4f},LR={lr:e},EPOCH={epoch:3},ENCODER={encoder},HIDDEN CHANNELS=[{chan[0]}-" \
                     f"{chan[1]}],"\
                     f"NUM LAYERS={num_lay},L={l},BETA={beta},SEED={i}"
            results = []
