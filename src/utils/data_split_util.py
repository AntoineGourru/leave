...
def leave_fraction_out(data, test_set_frac=0.2, directed=False, **_kwargs):
    if test_set_frac == 0:
        ...
        test_labels_pos = np.ones(test_data_pos.shape[0], np.bool_)
        test_labels_neg = np.zeros(test_data_neg.shape[0], np.bool_)
    ...
    # Generate negative edges of the same proportion as the train set.
    train_data_neg, train_neg_corr_factor = generate_negative_edges(data, nb_neg_per_data_edge=1-test_set_frac,
                                                                  directed=directed)
    ...
    # Create labels for the edges and concatenate them.
    train_labels_pos = np.ones(train_data.shape[0], np.bool_)
    train_labels_neg = np.zeros(train_data_neg.shape[0], np.bool_)
    train_data = np.concatenate((train_data, train_data_neg))
    ...
    return (train_data, train_labels, train_neg_corr_factor), (test_data, test_labels, test_neg_corr_factor)
...
def quick_split(data, test_set_fraction=0.51, directed=False):
    ...
    a = triu(nx.adjacency_matrix(G, nodelist=sorted(prepped_graph.nodes())))
...
def generate_negative_edges(data, nb_neg_per_data_edge=None, directed=True):
    ...
    while current_nb_negative_samples < nb_negative_samples:
        ...
        if not directed:
            lhs_samples, rhs_samples = np.unravel_index(actual_negative_lin_idx, shape=(nb_lhs_ids, nb_rhs_ids))
...
def prepare_pairs(data, sens_attr,
                  link_labels):
    # make pairs according to sens attr, sens-attr should be protS which is a dict (node:sens-attr)
    # Make input pairs for the model based on sensitive attribute
    pos_pairs = []
    neg_pairs = []
    for gen in data:
    ...
    neg_pairs = random.choices(neg_pairs,
                               k=len(pos_pairs))  # get equal number of positive and negative samples for training
