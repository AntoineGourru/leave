...
class DataLoader:
    def __new__(cls, dataset_name='', **kwargs):
        ...
        if dataset_name == "polblogs":
            from data_loader import PolBlogsLoader
            return super().__new__(PolBlogsLoader)
        elif dataset_name == 'lastfm':
            from data_loader import LastFMLoader
            return super().__new__(LastFMLoader)
        elif dataset_name == 'facebook':
            from data_loader import FacebookLoader
            return super().__new__(FacebookLoader)
        elif dataset_name == 'cv':
            from data_loader import CVLoader
            return super().__new__(CVLoader)
        elif dataset_name == 'pokec':
            from data_loader import PokecLoader
            return super().__new__(PokecLoader)
        else:
